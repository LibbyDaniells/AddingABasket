...
pw <- 0.3
q0 <- 0.2
cut_off <- c(rep(0.9034,K-1),0.90211)

t_scenarios <- list(t1,t2,t3)
Method3TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K,run,p.old,pw,q0,cut_off,mc.cores=8)
save(Method3TruthVaried,file='Method3TruthVaried15Sim1.RData')

p.old <- rep(0.4,K-1)
Method3TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K,run,p.old,pw,q0,cut_off,mc.cores=8)
save(Method3TruthVaried,file='Method3TruthVaried15Sim2.RData')

p.old <- c(0.4,0.4,0.2,0.2)
Method3TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K,run,p.old,pw,q0,cut_off,mc.cores=8)
save(Method3TruthVaried,file='Method3TruthVaried15Sim3.RData')

p.old <- c(0.4,0.3,0.3,0.2)
Method3TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K,run,p.old,pw,q0,cut_off,mc.cores=8)
save(Method3TruthVaried,file='Method3TruthVaried15Sim4.RData')
