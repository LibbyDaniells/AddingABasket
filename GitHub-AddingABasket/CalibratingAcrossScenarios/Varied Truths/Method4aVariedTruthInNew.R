...
VaryingTruthOC <- function(t,n,K1,K2,run,p.old,pw,q0,cut_off_exnex,cut_off_new){
  K <- K1+K2
  Fun <- function(x){
  ...
  for(j in 1:run){
    ...
    samplesEXNEX <- coda.samples(jags.fit,variable.names = c('p'),n.iter=100000,silent=TRUE)
    samplesEXNEX <- as.data.frame(samplesEXNEX[[1]])
    pmat <- as.matrix(samplesEXNEX[,(K1+1):K])
    point_estimates[j,(K1+1):K] <- colMeans(pmat)
    hypo[j,(K1+1):K] <- as.integer(apply(pmat,2,Fun)>cut_off_new)
  }
...
cut_off_new <- rep(0.90211,K2)

t_scenarios <- list(t1,t2,t3)
Method4TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K1,K2,run,p.old,pw,q0,cut_off_exnex,cut_off_new,mc.cores=8)
save(Method4TruthVaried,file='Method4TruthVaried15Sim1.RData')

p.old <- rep(0.4,K1)
Method4TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K1,K2,run,p.old,pw,q0,cut_off_exnex,cut_off_new,mc.cores=8)
save(Method4TruthVaried,file='Method4TruthVaried15Sim2.RData')

p.old <- c(0.4,0.4,0.2,0.2)
Method4TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K1,K2,run,p.old,pw,q0,cut_off_exnex,cut_off_new,mc.cores=8)
save(Method4TruthVaried,file='Method4TruthVaried15Sim3.RData')

p.old <- c(0.4,0.3,0.3,0.2)
Method4TruthVaried <- mclapply(t_scenarios,VaryingTruthOC,n,K1,K2,run,p.old,pw,q0,cut_off_exnex,cut_off_new,mc.cores=8)
save(Method4TruthVaried,file='Method4TruthVaried15Sim4.RData')